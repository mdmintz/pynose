...
test in the test.capturedLogging attribute, and displayed with the error
failure output. It is enabled by default but can be turned off with the option
`--nologcapture`.
You can filter captured logging statements with the `--logging-filter` option.
If set, it specifies which logger(s) will be captured; loggers that do not
match will be passed. Example: specifying `--logging-filter=sqlalchemy,myapp`
...
class LogCapture(Plugin):
    ...
    def start(self):
        self.handler = MyMemoryHandler(
            self.logformat, self.logdatefmt, self.filters
        )
